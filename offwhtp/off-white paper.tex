...
\newglossaryentry{bit}{name={bit}, description={The smallest unit of electronic data storage: there are eight bits in one byte. It is common to use bits when referring to cryptographic hashing algorithms (e.g. 160-bits instead of 20-bytes).}}
\newglossaryentry{storage_root}{name={storage_root}, description={One aspect of an \textsc{account's state}: this is the hash of the trie\footnote{A particular path from root to leaf in the state database} that decides the \textsc{storage contents} of the account}}
\newglossaryentry{interfacer}{name={interfacer}, description={A person or a piece of software that is able to interface to an Ethereum node. This denotes anyone or anything that can effect the state of the blockchain. Since Ethereum's state is effected only through actions made by accounts, every factual  interfacer with Ethereum has at least one account.
...
\justify
	...
	\section{Native Currency \& Mining}
	   ...
	   \midrule
	   \texttt{presentstate(total.difficulty)} & Total difficulty of \textsl{this block}. \\
	   \texttt{presentstate(block.parent)}     & \textsl{This block's} parent block. \\
